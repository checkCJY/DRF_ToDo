...
1) 프론트에서 /me/ 요청
   → header에 JWT access token 포함해서 보냄

2) DRF가 토큰 검증
   → 토큰 유효하면 request.user에 User 객체 자동 세팅
   → 토큰 없거나 만료되면 401 반환

3) permission_classes = [IsAuthenticated] 체크
   → request.user가 인증된 사용자인지 확인
   → 아니면 401 차단

4) get() 실행
   → request.user에서 id, username, email 꺼내서 JSON으로 반환


핵심은 JWT 토큰 → DRF가 request.user 세팅 이 부분이야.
프론트는 토큰만 들고 있고, 토큰 안에 user 정보가 암호화되어 있어서 DRF가 복호화해서 request.user에 넣어주는 거
